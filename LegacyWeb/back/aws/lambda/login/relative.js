...
exports.handler = (event, context, callback) => {
    ...
    let env         = event.env,
        body        = event.body,
        ip          = event.ip,
        collection  = event.collection || "relatives",
        db
        ...
        if( user ){
            if( body.code ){
                ...
                if ( user.verificationCode == body.code ){
                    ...
                    return db.collection( collection ).updateOne({_id: user._id}, {$unset: {verificationCode: 1}, $push: {ip: ip} })
                    .then(result => {
