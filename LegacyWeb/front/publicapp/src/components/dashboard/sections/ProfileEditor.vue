	...
	.profile-editor
				...
				.profile-picture
					...
					.mobile-actions.d-block.d-md-none
						v-row
							v-col.pt-0.pr-0
								.action-row
									v-btn(@click="$refs.file.click()", block, color="#3d64d9", dark, tile)
										|Change
										v-icon(color="#fff", right)
											|mdi-sync
							v-col.pt-0.pl-0(v-if="profilePicture!='/assets/icons/avatar_generico_add.svg'")
								.action-row
									v-btn(@click="deletePicture", block, color="red", dark, tile)
										|Delete
					...
					.change-hover.d-none.d-md-block()
						.actions
							.action-row
								v-btn(@click="$refs.file.click()", x-large, color="#3d64d9", dark)
									|Change picture
									v-icon(color="#fff", right)
										|mdi-sync
							.action-row
								v-btn(@click="deletePicture", x-large, color="red", dark, v-if="profilePicture!='/assets/icons/avatar_generico_add.svg'" )
									|Delete picture
									v-icon(color="#fff", size="40", right)
										|mdi-delete
					...
					v-form(ref="userInfoForm")
						...
						v-row
							v-col(md="6", cols="12")
								...
								v-text-field(
									...
									required)
								v-select(:items="['Male', 'Female']", outlined, label="Gender", v-model="profile.gender")
							v-col(md="6", cols="12")
								v-text-field(
						...
						v-row
							v-col(md="4", cols="12", offset-md="4")
								v-btn.white-btn-text(color="#3d64d9", :loading="status.updating", block, @click="updateInfo", 
							:small="small",
							:x-small="xsmall")
									|Update information
		...
		v-dialog.password-reset-dialog(v-model="showReset", max-width="700", persistent)
			v-card.password-reset-card
				...
				v-card-actions
					v-spacer
					v-btn.white-btn-text(color="#F44336", @click="showReset = false")
						|Cancel
					v-btn.white-btn-text(color="#3d64d9", :loading="status.resetting", @click="resetPassword")
						|Change password
</template>
		...
		methods: {
					...
					if( filetypes.includes( filetype ) ){
						...
						if( file.size < 3000000 ){
							...
							.then( response => {
								...
								url = this.baseApi + this.env + "relative-user"								
			...
			resetPassword: function() {
				if( this.validateResetPassword() ){
					...
					let url = this.baseApi + this.env + "relative-user/password-change"
					this.axios.post(url, body)
					.then( response => {
			...
			updateInfo: function() {
				...
				if( this.$refs.userInfoForm.validate() ){
					...
					let url = this.baseApi + this.env + "relative-user"
