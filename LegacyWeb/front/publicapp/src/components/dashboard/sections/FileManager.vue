<template lang="pug">
	.file-manager-panel
			...
			tr
				td
					.subsection-header-icon
						img(src="/assets/icons/ic_ddocument_lg.svg")
				td
					.subsection-header-posticon
						img(src="/assets/icons/ic_header_ddocument.svg")
				td
					.subsection-title
						|Files
				td
					.subsection-description
						|See all you documents
		...
		.content
			...
			v-data-table.others-datatable(:headers="otherTableHeaders", :items="otherFiles", :loading="loadingFiles", loading-text="Loading files", no-data-text="No files available")
						...
						v-tooltip(top, v-if="item.isWord")
							template(v-slot:activator="{ on }")
									v-icon.mr-1.ml-1(
										...
										v-on="on",
										alt="View document",
										@click="generateDocURL(item.url)")
										|mdi-eye-outline
							span
								|View document
						v-tooltip(top, v-else-if="item.isPdf")
							template(v-slot:activator="{ on }")
									v-icon.mr-1.ml-1(
										small,
										v-on="on",
										alt="View document",
										@click="generatePdfURL(item.url)")
										|mdi-eye-outline
							span
								|View document
						v-tooltip(top, v-else="item.isImage")
							template(v-slot:activator="{ on }")
									v-icon.mr-1.ml-1(
										small,
										v-on="on",
			...
			.clearfix.d-none(ref="viewer", v-viewer="{rotatable: false, scalable: false, movable: true}")
				img(:src="imageFocus")
</template>
			...
			.v-data-table
				.v-data-table__wrapper
					...
					&::-webkit-scrollbar-thumb
						background-image: -webkit-gradient(linear,left bottom,left top,color-stop(0.44, #548954),color-stop(0.72, #548954),color-stop(0.86, #385c38))
				table
					width: 250%
					overflow-x: scroll
					margin-top: 110px
