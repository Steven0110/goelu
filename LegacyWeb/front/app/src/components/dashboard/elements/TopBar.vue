	...
	.top-bar
		.desktop.d-none.d-md-block
			.return(v-show="returnable")
				v-icon.full-white(color="white", dark, size="30", @click="back")
					...
					v-list
						...
						v-list-item(@click="logout")
							...
							v-list-item-title
								|{{ $t("logout") }}						
			.profile-image
				img(:src="profilePicture", @click="$router.push({path: '/dashboard/profile'}).catch(err => {})")
			.name
				span(@click="$router.push({path: '/dashboard/profile'})")
					|{{ userName }}
		...
		methods: {
			back: function(){
				if(this.$route.query.keep)
					this.$router.push({path: '/dashboard/calendar', query: {keep: "true"}})
				else{
