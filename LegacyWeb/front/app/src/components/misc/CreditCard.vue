<template lang="pug">
	#app.wrapper
		.card-form
									...
									transition(name='slide-fade-up')
										.card-item__name(v-if='cardName.length', key='1')
											transition-group(name='slide-fade-right')
												span.card-item__nameItem(v-for="(n, $index) in cardName.replace(/\s\s+/g, ' ')", v-if='$index === $index', v-bind:key='$index + 1')
													|{{n}}
										.card-item__name(v-else, key='2')
											|{{$t('full-name')}}
								.card-item__date(ref='cardDate')
									label.card-item__dateTitle(for='cardMonth')
										|{{ $t('card-expires') }}
			...
			.card-form__inner
				.card-input
					label.card-input__label(for='cardNumber')
						|{{ $t('card-number') }}
					input#cardNumber.card-input__input(type='text', v-mask='generateCardNumberMask', v-model='cardNumber' v-on:focus='focusInput', v-on:blur='blurInput', data-ref='cardNumber', autocomplete='off', @input="update")
				.card-input
					label.card-input__label(for='cardName')
						|{{ $t('card-holder') }}
					input#cardName.card-input__input(type='text', v-model='cardName', v-on:focus='focusInput', v-on:blur='blurInput', data-ref='cardName', autocomplete='off', @input="update")
				.card-form__row
					.card-form__col
						.card-form__group
							label.card-input__label(for='cardMonth')
								|{{ $t('card-expiration-date') }}
							select#cardMonth.card-input__input.-select(v-model='cardMonth', v-on:focus='focusInput', v-on:blur='blurInput', data-ref='cardDate', @change="update")
								option(value='' disabled='' selected='')
									|{{ $t('card-month') }}
								option(v-bind:value="n < 10 ? '0' + n : n", v-for='n in 12', v-bind:disabled='n < minCardMonth', v-bind:key='n')
									|{{n &lt; 10 ? &apos;0&apos; + n : n}}
							select#cardYear.card-input__input.-select(v-model='cardYear', v-on:focus='focusInput', v-on:blur='blurInput', data-ref='cardDate', @change="update")
								option(value='', disabled='', selected='')
									|{{ $t('card-year') }}
					...
					.card-form__col.-cvv
						.card-input
							label.card-input__label(for='cardCvv')
								|CVV
							input#cardCvv.card-input__input(type='text', v-mask="'####'", maxlength='4', v-model='cardCvv', v-on:focus='flipCard(true)', v-on:blur='flipCard(false)', autocomplete='off', @input="update")
