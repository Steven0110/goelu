<template lang="pug">
    .audio-recorder
        .audio-container
            audio.video-js.vjs-default-skin(id="myAudio", playsinline)
            loader(position="absolute", background="transparent", v-show="status.saving", image="/assets/icons/loading.gif")
            ...
            v-row
                v-col(cols="12")
                    v-btn.mobile-done-button.d-block.d-md-none.elevation-0(
                        ...
                        @click="updateFile")
                        |{{$t('buttom-save-audio')}}
        .record-audio-calendar(v-show="show.calendar")
            .outter-container(:style="fixedWidth")
                .inner-container
                    scheduler(v-model="event", @cancel="show.calendar = false, file.relationship.type = 'now'", @close="show.calendar = false", @update="updateCalendar", from="audio", ref="scheduler")
</template>
    ...
    export default {
        ...
        mounted() {
            ...
            this.player.on('deviceError', () => {
                ...
                this.$swal("Microphone not found", "Be sure to install the proper drivers for your microphone in your computer.", "error")
            });
        },
        filters: {
        ...
        methods: {
            ...
            updateFile: function() {
                ...
                if( this.$refs.fileInfo.validate() ){
                    ...
                    .then(response => {
                        ...
                        return this.$swal("Done!", "The audio was saved successfully", "success")
                    })
                    .then(() => {
                        this.reset()
            ...
            redirectNewGroup: function() {
                this.$router.push({path: '/dashboard/contacts', query: {returnable: "true", newGroup: "true", from: "audio"}})
            },
            redirectNewContact: function() {
                this.$router.push({path: '/dashboard/contacts', query: {returnable: "true", newContact: "true", from: "audio"}})
            },
