			...
			v-col.pt-0(cols="12", md="5")
				v-row
					v-col.forgot-panel(cols="12")
						v-row
							v-col.forgot-topbar(cols="1", xl="1", lg="2", md="2")
								v-icon(size="30", @click="back")
									|$vuetify.icons.cback
						v-row
							v-col.pt-0(cols="12", md="8", offset-md="2", style="position: relative;padding-bottom: 40px")
								p.logo
		...
		methods: {
			proceed: function(){
				if( this.validate() ){
					...
					let url = this.baseApi + this.env + "user/forgot"
